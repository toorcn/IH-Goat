...
export function ReviewBoard({ context }: { context: ClientContext }) {
    ...
    try {
      ...
      setActionNotices((current) => ({
        ...
        [action.id]: {
          ...
          tone: "signal",
          message:
            data.sendMode === "advisor_approval_required"
              ? "Approved for Sarah's review queue. No client-facing message was sent."
              : "Approved locally. No client-facing transport is configured."
        }
      }));
    } catch (caught) {
      setActionNotices((current) => ({
        ...
        [memory.id]: {
          ...
          message:
            data.writeMode === "neo4j" && data.saved
              ? "Saved to Neo4j. Refresh the client profile to see it in approved/recent memory."
              : `Approved for demo continuity. ${data.reason ?? "Neo4j is not configured."}`
        }
      }));
    } catch (caught) {
      setMemoryNotices((current) => ({
  ...
  return (
    <div className="grid gap-5 xl:grid-cols-[minmax(0,0.9fr)_minmax(0,1.1fr)]">
        ...
        <div className="space-y-3 text-sm leading-6 text-muted">
          ...
          <p>
            {openMemories.length > 0
              ? `${context.advisor.name} should review open items: ${openMemories
                  .map((memory) => memory.title)
                  .join(", ")}.`
              : `${context.advisor.name} has no open memory items in the selected context.`}
          </p>
        </div>
      ...
      <Panel title="Memory Updates" eyebrow="Advisor-approved proposals" className="xl:col-span-2">
