...
import type { ActionItem, ClientContext, ExtractedMemory, Meeting } from "@/lib/types";
...
const initialRecap =
  "The meeting centered on a family milestone, an unresolved estate planning need, and the client wanting a clearer path forward. The advisor promised a practical resource and should close the loop with a specialist introduction while the topic is still fresh.";

export function PostMeetingWrapUp({ context, meeting }: { context: ClientContext; meeting: Meeting }) {
      ...
      <section className="surface-enter overflow-hidden rounded-[1.6rem] border border-line/80 bg-panel shadow-diffusion">
        <div className="grid gap-5 p-4 sm:p-5 xl:grid-cols-[minmax(0,1.15fr)_minmax(320px,0.85fr)]">
          <div className="flex min-w-0 flex-col justify-between gap-5">
            <div>
              ...
              <h1 className="mt-4 max-w-4xl text-3xl font-semibold leading-tight tracking-tight text-ink sm:text-4xl">
                The close-loop is ready for {context.client.name}.
              </h1>
          ...
          <div className="grid gap-3 rounded-[1.25rem] border border-line bg-paper/85 p-3 sm:grid-cols-2">
            <HeaderMetric label="Client" value={context.client.name} icon={<MessageSquareText className="h-4 w-4" />} />
            <HeaderMetric label="Advisor" value={context.advisor.name} icon={<Sparkles className="h-4 w-4" />} />
      ...
      <div className="grid gap-5 xl:grid-cols-[minmax(0,0.9fr)_minmax(0,1.1fr)]">
        ...
        <section className="surface-enter rounded-[1.6rem] border border-line/80 bg-panel p-4 shadow-diffusion sm:p-5">
          ...
          <p className="mt-3 text-sm leading-6 text-muted">
            Next time, remember to check whether the client reviewed the resource, whether the specialist
            connection happened, and whether the original concern has been resolved.
          </p>
          <div className="mt-4 grid gap-2 sm:grid-cols-2">
            {[
              "Ask if the client reviewed the estate planning guide.",
              "Confirm whether the specialist introduction happened.",
              "Revisit the unresolved will planning concern.",
              "Check if the client feels clearer about the family transition."
            ].map((item) => {
              return (
                <div
                  key={item}
...
function buildMemoryUpdates(context: ClientContext): WrapMemory[] {
  ...
  const actionMemories = context.actions.slice(0, 2).map((action, index): WrapMemory => ({
    ...
    label: "Promise made",
    summary: `${context.advisor.name} should close the loop on: ${action.title}.`,
...
function buildFollowUps(context: ClientContext): FollowUpAction[] {
  const actionCards = context.actions.map((action, index): FollowUpAction => ({
    ...
    reason: action.actionType === "introduction"
      ? "The client has an estate planning need that a trusted specialist can help with."
      : action.draftText
        ? "The advisor promised a concrete follow-up during the meeting."
        : "The meeting left an open item that should not wait until the next review.",
    priority: index === 0 ? "High" : index === 1 ? "Medium" : "Low",
    nextStep: action.actionType === "introduction"
      ? "Draft a warm introduction and let the advisor choose the specialist before sending."
      : action.draftText
        ? "Review the generated message and send it today while the meeting context is fresh."
        : "Create a dated reminder so this concern appears before the next client interaction.",
    dueAt: action.dueAt,
  ...
  return [
    {
      ...
      title: "Send promised planning resource",
      reason: "The meeting created a clear follow-up that should be closed today.",
      priority: "High",
      nextStep: "Review the draft and send the resource after advisor approval.",
      dueAt: "2026-06-21"
    }
  ];
}

function buildRecommendations(context: ClientContext, followUps: FollowUpAction[]): Recommendation[] {
  return [
    {
    ...
    {
      id: "rec-specialist",
      title: "Draft a warm specialist introduction",
      why: `${context.client.name} has an open planning need that should move from concern to concrete next step.`,
      priority: "Medium"
    },
    {
      id: "rec-check-in",
...
function buildFollowUpDraft(context: ClientContext, followUps: FollowUpAction[]) {
  ...
  return `Hi ${context.client.name}, thank you for meeting today. As discussed, I will ${firstAction}. ${
    specialistAction ? "I can also prepare a warm specialist introduction for the planning topic we covered. " : ""
...
function buildMemoryPath(context: ClientContext, memories: WrapMemory[]) {
  const topMemory = memories[0]?.category ?? "Memory update";
  return [
    { title: context.client.name, detail: "Client context", icon: <MessageSquareText className="h-4 w-4" /> },
    ...
    { title: "Next check-in", detail: "Advisor prompted", icon: <Bell className="h-4 w-4" /> }
  ];
}
