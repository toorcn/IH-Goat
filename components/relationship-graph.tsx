...
export function RelationshipGraph({
  ...
  return (
    <Panel title="Network" eyebrow={source === "neo4j" ? "Neo4j relationships" : "Demo relationships"}>
      <div className="mb-4 flex flex-wrap items-center gap-2">
        <Badge tone={source === "neo4j" ? "signal" : "amber"}>
          {source === "neo4j" ? "Retrieved from Neo4j" : "Demo fallback"}
        </Badge>
...
function buildNodePositions(nodes: GraphNode[], edges: GraphEdge[]) {
  ...
  const clientNodes = orderedNodes.filter((node) => node.type === "Client");
  const advisorNodes = orderedNodes.filter((node) => node.type === "Advisor");
  const personNodes = orderedNodes.filter((node) => node.type === "Person");
  const referralNodes = orderedNodes.filter((node) => node.type === "ReferralOpportunity");
  const specialistNodes = orderedNodes.filter((node) => node.type === "Specialist");
  const otherNodes = orderedNodes.filter(
    (node) =>
      !clientNodes.includes(node) &&
      !advisorNodes.includes(node) &&
      !personNodes.includes(node) &&
      !referralNodes.includes(node) &&
      !specialistNodes.includes(node)
  );

  place(clientNodes, 50, 50, 0, 10, layout);
  place(advisorNodes, 18, 22, 0, 13, layout);
  place(personNodes, 22, 73, 0, 13, layout);
  place(referralNodes, 75, 24, 0, 12, layout);
  place(specialistNodes, 78, 72, 0, 14, layout);
...
function typeRank(type: GraphNode["type"]) {
  ...
  if (type === "ReferralOpportunity") return 3;
  return 4;
}
...
function nodeClassName(type: GraphNode["type"]) {
  if (type === "Client") return "border-signal/40 bg-signal";
  if (type === "Advisor") return "border-cobalt/30 bg-cobalt";
  if (type === "ReferralOpportunity") return "border-amber/40 bg-amber";
  if (type === "Specialist") return "border-rose/30 bg-rose";
