...
const intentKeywords: Record<Exclude<Intent, "unknown">, string[]> = {
  referral: ["referral", "introduce", "introduction", "specialist", "lawyer", "evelyn", "marcus"],
...
export function buildMemoryQueryVisualResponse(
  ...
  const base: Pick<
    MemoryQueryVisualResponse,
    "clientId" | "query" | "source" | "displayMode" | "citations" | "warning"
  > = {
  ...
  if (displayMode === "recommendation") {
    ...
    return {
      ...base,
      answer: introAction
        ? `${context.client.name} has an open estate-planning referral opportunity. The strongest next step is to use the pending introduction: ${introAction.title}.`
        : `${context.client.name} has an open referral need. Offer a warm introduction only after confirming he still wants specialist help.`,
      cards: [
        {
          id: referralMemory?.id ?? "referral-recommendation",
          eyebrow: "Recommended intro",
          title: introAction?.title ?? "Confirm specialist introduction",
          body:
            introAction?.draftText ??
            referralMemory?.summary ??
            "Confirm the specific estate-planning need, then propose a warm specialist introduction.",
          meta: referralMemory?.source ?? "Client memory"
        }
      ],
      graph: context.graph,
      actions: toSuggestedActions(
        introAction ? [introAction] : matchingActions.length > 0 ? matchingActions : context.actions.slice(0, 1),
        referralMemory?.summary ?? "Referral opportunity found in client memory."
      )
    };
  }
  ...
  if (displayMode === "graph") {
    return {
      ...base,
      answer: `${context.client.name}'s visible network includes ${context.graph.nodes
        .filter((node) => node.id !== context.client.id && node.id !== context.advisor.id)
        .map((node) => node.label)
        .join(", ")}.`,
      graph: context.graph,
  ...
  if (displayMode === "table") {
    ...
    return {
      ...
      answer:
        rows.length > 0
          ? `There are ${rows.length} relevant pending action${rows.length === 1 ? "" : "s"} for ${context.client.name}.`
...
function memoryMatchesIntent(memory: MemoryItem, intent: Intent) {
  ...
  if (intent === "relationship") return memory.category === "Relationship Mention" || memory.category === "Life Event";
...
function buildMissingInfoResponse(context: ClientContext, query: string): MemoryQueryVisualResponse {
  ...
  return {
    ...
    answer: `I do not see confirmed memory for "${query}" in ${context.client.name}'s current client record.`,
    citations: [],
    missingInfo: {
      title,
      reason: "The query did not match current memories, actions, or relationship graph evidence.",
      suggestedNextStep: `Ask ${context.client.name} about this during the meeting, then save it as a pending memory gap for advisor review.`
    },
    actions: [
      {
...
function summarizeMemories(context: ClientContext, memories: MemoryItem[]) {
  if (memories.length === 0) {
    return `No relevant memory was found for ${context.client.name}.`;
  }

  return `${context.client.name}: ${memories
    .slice(0, 3)
