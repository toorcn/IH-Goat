...
const meetingId = "meeting-2026-06-20-tan";
...
function checkExtractionHelper() {
  ...
  assert.equal(signals.suggestions.some((item) => item.id === "suggest-will"), true);
  assert.equal(signals.suggestions.some((item) => item.id === "suggest-referral"), true);
...
async function checkActionApprovalRoute() {
  ...
  assert.equal(payload.sendMode, "advisor_approval_required");
}
  ...
  return [
    {
      id: "e1",
      speaker: "client",
      text: "Jia En is excited about NUS and I still need to update my will.",
      timestamp: "2026-06-20T10:35:00+08:00"
    },
    {
      id: "e2",
      speaker: "client",
      text: "Please introduce me to Evelyn or a lawyer and send the guide.",
